...
export default function ImageGallery({ identity }) {
  ...
  useEffect(() => {
    async function fetchImages() {
      const response = await fetchData("get",`user/imagessoftour/${identity}`
      );
  ...
  async function handleDeleteImage(id) {
    await fetchData("delete",`/user/deleteimagesoftour/${id}`);
  ...
  async function handleAddImage(event) {
    ...
    const response = await fetchData("post",`/user/addimagesoftour/${identity}`,
      {
